...
- 단계마다 필터를 적용하여 이미지 왼쪽위부터 오른쪽밑까지 밀어가며 곱하고 더하는 작업(컨볼루션)을거쳐 윤곽선, 질감 등의 특징을 추출한다. 
- 컨볼루션계층 : 이미지의 특징 추출 (특징맵 만듬, 필터신경망(커널) 여러개존재, 컨볼루션연산수행) -> 풀링계층 : 필터를 거친 여러 특징중 가장 중요한 특징을 골라냄 (특징맵의 크기 줄여 중요특징강조) 
- CNN은 이미지 추출하는 필터를 학습한다. 
